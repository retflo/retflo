---
coordinate: ECON.SERIALCAPTURE.1
tags: [serial-capture, oligarchic-capture, distributed-vulnerability, money-in-politics, anti-corruption, coordinated-wealth]
domain: econ
links:
  flow:
...
1. Concentrated wealth exists
2. Distributed governance has many decision-points
3. Each decision-point is cheaper to capture than a single centralized agency
4. Therefore distributed governance is more vulnerable to capture
...
1. **Public, equalized funding** — remove the money lever
2. **Strict transparency and auditability** — make capture visible and costly
3. **Hard limits on private financing** of governance functions
4. **Cross-jurisdiction anti-corruption enforcement** tied to federation membership
...
|---|---|---|
| "[Specific network] captured governance bodies one by one" | They exploited money-driven elections inside a centralized political economy. The vulnerability was money in politics, not localism. Address the actual mechanism. | **Fact** — accepts that money corrupts governance, conceding the problem is private wealth influence not local democracy |
| "Distributed = more targets = easier capture" | Only if the targets are starved and dependent on private money. With public funding, transparency, and anti-corruption enforcement, capture becomes harder — no single pipeline buys the whole map. | **Contested** — concedes both architectures face capture; whether distributed design resists coordinated wealth better than centralized defenses is the open wager, local elite capture being the live counterevidence — the framework reads the record as indicting money-in-politics, not localism |
| "Professional institutions resist capture through culture" | Professional militaries stage coups. Regulatory agencies get captured. Institutional culture does not immunize against power — it often normalizes it. | **Fact** — accepts that institutional capture is real and documented, conceding professionalization is not a reliable safeguard |
